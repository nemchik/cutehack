...
//                            'index.php',  -> not important ?!? :|
    ...
    if (!$handle = opendir("./data")) {
        die("<center>Can not open directory ./data ");
    }
...
elseif ($action == "doinstall") {
    if (!$reg_username or !$reg_password1 or !$url or $reg_password1 != $reg_password2) {
        die("Error !!!<br /> You didn't fill some required field or the passwords did not match");
    }
    ...
    $views_file = fopen("./data/counter.db.php", "w") or die("Sorry but installer can not write to file <b>./data/counter.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fwrite($views_file, "<?PHP die(\"You don't have access to open this file !!!\"); ?>\n");
    fclose($views_file);

    $flood_file = fopen("./data/flood.db.php", "w") or die("Sorry but installer can not write to file <b>./data/flood.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fwrite($flood_file, "<?PHP die(\"You don't have access to open this file !!!\"); ?>\n");
    fclose($flood_file);

    $pro_file = fopen("./data/profiles.db.php", "w") or die("Sorry but installer can not write to file <b>./data/profiles.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fwrite($pro_file, "<?PHP die(\"You don't have access to open this file !!!\"); ?>\n");
    fclose($pro_file);

    $xfields_file = fopen("./data/xfields.db.php", "w") or die("Sorry but installer can not write to file <b>./data/xfields.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fwrite($xfields_file, "<?PHP die(\"You don't have access to open this file !!!\"); ?>\n");
    fclose($xfields_file);

    $xfdata_file = fopen("./data/xfieldsdata.db.php", "w") or die("Sorry but installer can not write to file <b>./data/xfieldsdata.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fwrite($xfdata_file, "<?PHP die(\"You don't have access to open this file !!!\"); ?>\n");
    fclose($xfdata_file);

    $news_file = fopen("./data/news.db.php", "a") or die("Sorry but installer can not write to file <b>./data/news.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fclose($news_file);

    $comm_file = fopen("./data/comments.db.php", "a") or die("Sorry but installer can not write to file <b>./data/comments.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fclose($comm_file);

    $category_file = fopen("./data/category.db.php", "a") or die("Sorry but installer can not write to file <b>./data/category.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fclose($category_file);

    $pfltmp_file = fopen("./data/protemp.db.php", "a") or die("Sorry but installer can not write to file <b>./data/protemp.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fclose($pfltmp_file);

    $ipban_file = fopen("./data/ipban.db.php", "a") or die("Sorry but installer can not write to file <b>./data/ipban.db.php</b>,<br />please check if file exist and if it is properly CHMOD`ed");
    fclose($ipban_file);
...
\$config_user_image_upload = "no";

\$config_user_image_delete = "no";

\$config_images_gd = "no";

\$config_mail_admin_address = "$reg_email";

\$config_register_allow = "no";

\$config_register_level = "4";

\$config_register_multimail = "no";

\$config_register_mailpass = "yes";

\$config_register_mailadmin = "yes";

\$config_register_filter = "admin, owner";

\$config_timestamp_active = "d M Y";

\$config_timestamp_archive = "d M";

\$config_dateheader_archive = "Y";

\$config_reverse_active = "no";

\$config_format_archive = "b";

\$config_prospective_posting = "no";

\$config_allow_short = "yes";

\$config_allow_full = "yes";

\$config_short_full = "short";

\$config_max_story_length = "200";

\$config_archive_level = "1";

\$config_allow_html_articles = "2";
...
\$config_allow_disable_comments = "2";

\$config_mail_allow_comments = "2";

\$config_mail_admin_comments = "0";
