            ...
            if($my_names[$news_arr[1]]){ $my_author = $my_names[$news_arr[1]]; }
                ...
                if($news_arr[5] != ""){$output = str_replace("{avatar}", "<img alt=\"\" src=\"$news_arr[5]\" style=\"border: none;\" />", $output); }
                    ...
                    if($config_comments_popup == "yes"){
                                ...
                                $output = str_replace("[com-link]","<a href=\"$config_http_script_dir/index.php?subaction=showcomments&amp;id=$news_arr[0]&amp;archive=$archive&amp;start_from=$my_start_from&amp;ucat=$news_arr[6]&amp;$user_query\">", $output);
                        }
...
if($allow_comments){
    ...
    if(isset($comm_start_from) and $comm_start_from != "" and $comm_start_from > 0){
        ...
        $prev_next_msg = preg_replace("'\[prev-link\](.*?)\[/prev-link\]'si", "<a href=\"$config_http_script_dir/index.php?comm_start_from=$prev&amp;archive=$archive&amp;subaction=showcomments&amp;id=$id&amp;$user_query\">\\1</a>", $prev_next_msg);
    ...
    for($j=1;$j<=$pages_count;$j++){
        if($pages_start_from != $comm_start_from){ $pages .= "<a href=\"$config_http_script_dir/index.php?comm_start_from=$pages_start_from&amp;archive=$archive&amp;subaction=showcomments&amp;id=$id&amp;$user_query\">$j</a> "; }
    ...
    if($comm_per_page < $total_comments and $comment_number < $total_comments){
        $prev_next_msg = preg_replace("'\[next-link\](.*?)\[/next-link\]'si", "<a href=\"$config_http_script_dir/index.php?comm_start_from=$comment_number&amp;archive=$archive&amp;subaction=showcomments&amp;id=$id&amp;$user_query\">\\1</a>", $prev_next_msg);
