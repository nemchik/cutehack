...
if($allow_add_comment){
        ...
        if($is_logged_in && $my_names[$news_arr[1]]){ $name = $my_names[$name]; }
        $name = trim($name);
        $mail = trim($mail);
    ...
    if($config_auto_wrap > 1){
        ...
        foreach($comments_arr as $null => $line){
            ...
            if(strlen($name) > $config_auto_wrap){ $name = substr($name, 0, $config_auto_wrap)." &hellip;"; }
    ...
    if($mail == "" or trim($mail) == ""){ $mail = "none"; }
    else{ $ok = FALSE;
    ...
    {
    	if(eregi("<\?",$old_comments_line)) continue;
...
if ($news_arr[0] == $old_comments_arr[0] && isset($news_arr[2]) && $news_arr[2] != "") {
 ...
 if ($mail_on_comment_run == true) { cute_mail($my_mails[$news_arr[1]], "Your post has received a comment", "$mail_name has commented on your post: \n $mail_title $send_them_mail \n\n Comment: \n $mail_comment"); }
 if (($config_mail_admin_comments == "1" && ($archive == "" || !$archive)) || $config_mail_admin_comments == "2") { cute_mail($config_mail_admin_address, "$news_arr[1]'s post has received a comment", "$mail_name has commented on $news_arr[1]'s post: \n $mail_title $send_them_mail \n\n Comment: \n $mail_comment"); }
}
}}
...
foreach ($article_counter as $null => $counter_line) {
	if(eregi("<\?",$counter_line)) continue;
...
if (!isset($template) || $template == "") { include ("$cutepath/data/Default_dh.tpl.php"); }
else { include ("$cutepath/data/".$template."_dh.tpl.php"); }
$template_full = str_replace("{archiveheader}","", $template_full);
 $output = $template_full.$output;
if (!isset($template) || $template == "") { include ("$cutepath/data/Default.tpl.php"); }
else { include ("$cutepath/data/".$template.".tpl.php"); }
}}
// Date Header
$article_counter = file("$cutepath/data/counter.db.php");
foreach ($article_counter as $null => $counter_line)
...
if (!function_exists(tagdate)) { function tagdate($match) { global $news_arr; return date($match[1], $news_arr[0]+($config_date_adjust*60)); } }
...
if ($config_reverse_comments == "yes") { $output = str_replace("{comment-number}", $total_comments - $comment_number,$output); }
else { $output = str_replace("{comment-number}", $comment_number+1,$output); }
...
// Valid Amps
    ...
    }else{ if ($config_prevnext_firstlast == "yes") { $prev_next_msg = preg_replace("'\[prev-link\](.*?)\[/prev-link\]'si", "\\1", $prev_next_msg); } else { $prev_next_msg = preg_replace("'\[prev-link\](.*?)\[/prev-link\]'si", "", $prev_next_msg); } $no_prev = TRUE; }
...
// Current/Total Pagination v1.0 - End addblock
    ...
    }else{ if ($config_prevnext_firstlast == "yes") { $prev_next_msg = preg_replace("'\[next-link\](.*?)\[/next-link\]'si", "\\1", $prev_next_msg); } else { $prev_next_msg = preg_replace("'\[next-link\](.*?)\[/next-link\]'si", "", $prev_next_msg); } $no_next = TRUE;}
...
if(!$is_logged_in){
        ...
        if ($_COOKIE["rememberpw"] || !$config_use_sessions) $check = $check." checked=checked ";
        if (!$config_use_sessions) $check = $check." disabled=disabled ";
...
foreach ($news_arr as $null => $news_value) { $all_news[$a] .= $news_value."|"; }
$all_news[$a] .= $news_c[$a]."||";
}
// Top Comments
...
foreach ($count_file as $null => $count_line) {
	if(eregi("<\?",$count_line)) continue;
 if (stristr($count_line, $news_arr[0])) {
  $news_v[$a] = $news_arr[1];
 }
}
if ($news_v[$a] == "") { $news_v[$a] = "0"; }
foreach ($news_arr as $null => $news_value) { $all_news[$a] .= $news_value."|"; }
$all_news[$a] .= $news_v[$a]."||";
}
// Top Views
...
if ($config_prospective_posting == "yes" && ($news_arr[0]+($config_date_adjust*60)) > time()) { continue; }
// <!-- End ModifyTime v2.0 -->
// Negative Categories v1.0 - changeblock - Old
