...
if (substr($config_http_script_dir, -1) == "/") { $config_http_script_dir = substr($config_http_script_dir, 0, -1); }
...
if ($config_user_image_upload == "yes") { $config_path_image_upload = $config_path_image_upload."/".strtolower(str_replace(" ","",$username)); }
...
if ($config_register_allow == "no") { echo "User Registration is not enabled."; } else {
echo "
...
"; if ($config_register_mailpass == "no") { echo "
    <tr>
...
if ($config_register_allow == "no") { msg("error","Error !!!", "User Registration is not enabled."); } else {
$filter = explode(",", str_replace(" ", "", str_replace(",,", ",", $config_register_filter)));
if ($config_register_level=="4") $level_mail="Commenter"; if ($config_register_level=="3") $level_mail="Journalist"; if ($config_register_level=="2") $level_mail="Editor"; if ($config_register_level=="1") $level_mail="Administrator";
if ($config_register_mailpass == "yes") { $regpass = makeRandomPassword(); $conpass = $regpass; }
$reguser = trim($reguser, " \t\n\r\0"); $regpass = trim($regpass, " \t\n\r\0"); $regmail = trim($regmail, " \t\n\r\0"); $conpass = trim($conpass, " \t\n\r\0"); $conmail = trim($conmail, " \t\n\r\0");
if (!$reguser) { msg("error","Error !!!", "Username can not be blank"); }
if (!$regpass || !$conpass || $regpass != $conpass) { msg("error","Error !!!", "Password can not be blank, both fields must match"); }
if (!$regmail || !$conmail || $regmail != $conmail) { msg("error","Error !!!", "Email can not be blank, both fields must match"); }
if (!preg_match("/^[\.A-z0-9_\-]+[@][A-z0-9_\-]+([.][A-z0-9_\-]+)+[A-z]{1,4}$/i", $regmail)) { msg("error","Error !!!", "Invalid Email."); }
if (preg_match("/[^\.A-z0-9_\-$]/i", $reguser, $matches)) { $match = ""; if ($matches) $match = "You entered '$matches[0]'."; msg("error","Error !!!", "Invalid Username. Must contain only A-Z, a-z, 0-9, underscores, dashes, or periods. $match"); }
if (preg_match("/[^\.A-z0-9_\-$]/i", $regpass, $matches)) { $match = ""; if ($matches) $match = "You entered '$matches[0]'."; msg("error","Error !!!", "Invalid Password. Must contain only A-Z, a-z, 0-9, underscores, dashes, or periods. $match"); }
foreach ($filter as $null => $block) { if (stristr($reguser, $block)) { msg("error","Error !!!", "Invalid Username. <b>\"$block\"</b> not allowed."); } }
...
foreach($all_users as $null => $user_line) {
	$user_arr = explode("|", $user_line);
	if(stristr("|".$user_arr[2]."|", "|".$reguser."|")){ msg("error", "Error", "This username is already taken"); }
	if($config_register_multimail=="no" && stristr("|".$user_arr[5]."|", "|".$regmail."|")){ msg("error", "Error", "This email address is registered to another user!"); }
}
...
if ($config_register_mailpass == "yes"){ cute_mail($regmail, "Registration at $SERVER_NAME", "$reguser, you have registered successfully. \n ---------- \n Your User Level is: $level_mail \n Your Password is: $regpass \n You can change this once you login. \n ---------- \n You can login here: $config_http_script_dir/ \n Thank You for Registering!"); }
if ($config_register_mailadmin == "yes") { cute_mail($config_mail_admin_address, "New User: $reguser", "$reguser has registered on your CuteNews system. \n The address used to register was: $regmail \n To delete this user go to the following address: $config_http_script_dir/index.php?mod=editusers&action=dodeleteuser&id=$add_time"); }

if (!isset($config_mail_admin_address) || $config_mail_admin_address == "") { $problem_contact = "an administrator"; }
else { $problem_contact = "<a href=\"mailto:".$config_register_mailadmin_address."\" target=\"_blank\">".$config_register_mailadmin_address."</a>"; }
if ($config_register_mailpass == "yes"){ msg("user", "$level_mail Added", "You have successfully registered as <b>\"$reguser\"</b>.<br />Your password has been emailed to <b>\"$regmail\"</b>.<br />If this information is wrong or you do not recieve your password please contact $problem_contact."); }
else { msg("user", "$level_mail Added", "You have successfully registered as <b>\"$reguser\"</b>.<br />Your password is <b>\"$regpass\"</b>.<br />Your email address is <b>\"$regmail\"</b>.<br />If this information is wrong please contact $problem_contact."); }
}}
...
function flogin(frm) {
	...
	<tr>
		...
		<td align=left valign=bottom>
			<input tabindex=3 type=checkbox class=checkbox name=rememberpw value=true<?PHP if($_COOKIE["rememberpw"] || !$config_use_sessions) echo " checked=checked"; ?><?PHP if(!$config_use_sessions) echo " disabled=disabled"; ?> onclick="document.login.enteredpw.focus();"> Remember Me
