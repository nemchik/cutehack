...
$config_path_image_upload        = dirname(__FILE__)."/data/upimages";
...
$all_users_db = file(dirname(__FILE__)."/data/users.db.php");
...
if(isset($config_skin) and $config_skin != "" and file_exists("./skins/${config_skin}.skin.php")){
        require_once(dirname(__FILE__)."/skins/${config_skin}.skin.php");
}else{
        $using_safe_skin = true;
        require_once(dirname(__FILE__)."/skins/default.skin.php");
}
...
if(isset($username))
{
    ...
    if(check_cute_login($username, $cmd5_password))
    {
        $cookie_logged = TRUE;
...
if($action == "dologin")
{
    ...
    if(check_cute_login($username, $md5_password)){
                $session_logged = TRUE;
    ...
    if(check_cute_login($_SESSION['username'], $_SESSION['md5_password'])){
...
if($session_logged == TRUE or $cookie_logged == TRUE){
    if($action == 'dologin'){
        ...
        $modified_users = fopen(dirname(__FILE__)."/data/users.db.php", "w");
