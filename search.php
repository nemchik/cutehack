...
HTML;
	if((count($cat_lines)-1) > 0){
        ...
        foreach($cat_lines as $null => $single_line){
  						$cat_arr = explode("|", $single_line);
  						if (strlen($cat_arr[1]) > 17) { $cat_arr['disp'] = substr($cat_arr[1], 0, 17 - 3) . '&hellip;'; }
...
if($dosearch == "yes")
{
    ...
    foreach($files_arch as $null => $file)
    {
        ...
        $archive = FALSE;
        if(ereg("([[:digit:]]{0,})\.news\.arch\.php", $file, $regs)){ $archive = $regs[1]; }
        $all_news_db = file("$file");
            ...
            if($do_date)
            {
                    if($date_from < $news_db_arr[0] and  $news_db_arr[0] < $date_to){ $fdate = TRUE; }else{ $fdate = FALSE; }
                        ...
                        if($fdate and $ftitle and $fuser and $fstory and $fcat){ $found_arr[$news_db_arr[0]] = $archive; }
    ...
    if(is_array($found_arr)){
        ...
        {
            if($archive){$all_news = file("$cutepath/data/archives/$archive.news.arch.php");}
            else{ $all_news = file("$cutepath/data/news.db.php"); }
