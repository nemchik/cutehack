...
if(!isset($template) or $template == "" or strtolower($template) == "default"){ require_once("$cutepath/data/Default.tpl.php"); }
else{
        if(file_exists("$cutepath/data/${template}.tpl.php")){ require_once("$cutepath/data/${template}.tpl.php"); }
...
// Archive Date Header
if ($show_ah == true) {
	if (!$config_dateheader_archive || $config_dateheader_archive == "") $config_dateheader_archive = "Y";
...
if (date($config_timestamp_archive,$first_timestamp) == date($config_timestamp_archive,$last_timestamp)) { $show_timestamp = $show_first; }
