...
if (isset($bgcolor) && $bgcolor != "") {
    $bg = "bgcolor=\"$bgcolor\"";
        ...
        if (!stristr($user_flags, "u")) {
            $u = "<td $bg>&nbsp;<u>Username</u>";
        }
        if (!stristr($user_flags, "n")) {
            $n = "<td $bg>&nbsp;<u>Nickname</u>";
        }
        if (!stristr($user_flags, "r")) {
            $r = "<td $bg>&nbsp;<u>Registration Date</u>";
        }
        if (!stristr($user_flags, "a")) {
            $a = "<td $bg>&nbsp;<u>Avatar</u>";
        }
        if (!stristr($user_flags, "p")) {
            $p = "<td $bg>&nbsp;<u>Posts</u>";
        }
        if (!stristr($user_flags, "l")) {
            $l = "<td $bg>&nbsp;<u>Access Level</u>";
        }
        if (!stristr($user_flags, "e")) {
            $e = "<td $bg>&nbsp;<u>EMail</u>";
        }
...
// Sort users v1.0 - End addblock
    ...
    foreach ($all_users as $null => $user_line) {
        $i++;
        $bg = "";
        if ($i%2 == 0 && isset($bgcolor) && $bgcolor != "") {
            $bg = "bgcolor=\"$bgcolor\"";
        }
        if (!eregi("<\?", $user_line)) {
            ...
            if ($user_arr[7]=="0") {
                $user_email = "<a href=mailto:$user_arr[5]>[send mail]</a>";
            } else {
                $user_email = "[hidden]";
            }
            if ($user_arr[8]!="") {
                $user_av = "<a href=$user_arr[8]>[click]</a>";
            } else {
                $user_av = "[none]";
            }
...
echo "</tr></table><form method=post action=\"$PHP_SELF?$user_query\" ><select name=sortus >";
    ...
    if (!stristr($user_flags, "l")) {
        ...
        echo " >Access Level</option>";
    }
echo "</select>
<select name=sortad >
<option value=d "; if ($_POST['sortad']=="d") {
    echo "selected";
} echo " >Descending Order</option>
<option value=a "; if ($_POST['sortad']=="a") {
    echo "selected";
} echo " >Ascending Order</option>
</select>
